...
class NARRE(nn.Module):
    ...
    def forward(self, datas):
        ...
        user_feature = self.user_net(user_reviews, uids, user_item2id)
        item_feature = self.item_net(item_reviews, iids, item_user2id)
...
class Net(nn.Module):
    ...
    def forward(self, reviews, ids, ids_list):
        ...
        # --------cnn for review--------------------
        feature = F.relu(self.cnn(reviews.unsqueeze(1))).squeeze(3)  # .permute(0, 2, 1)
        feature = F.max_pool1d(feature, feature.size(2)).squeeze(2)
        feature = feature.view(-1, r_num, feature.size(1))

        # ------------------linear attention-------------------------------
        rs_mix = F.relu(self.review_linear(feature) + self.id_linear(F.relu(u_i_id_emb)))
        att_score = self.attention_linear(rs_mix)
        att_weight = F.softmax(att_score, 1)
        r_fea = feature * att_weight
        r_fea = r_fea.sum(1)
