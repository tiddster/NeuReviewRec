...
class PredictionLayer(nn.Module):
    ...
    def forward(self, feature, user_id, item_id):
        if self.output == "fm" or "lfm" or "nfm":
...
class LFM(nn.Module):
    def __init__(self, dim, user_num, item_num):
        ...
        self.bias_users = nn.Parameter(torch.randn(user_num, 1))
        self.bias_items = nn.Parameter(torch.randn(item_num, 1))
    ...
    def init_weight(self):
        ...
        nn.init.uniform_(self.bias_users, 0.5, 1.5)
        nn.init.uniform_(self.bias_items, 0.5, 1.5)

    def rescale_sigmoid(self, score, a, b):
        return a + torch.sigmoid(score) * (b - a)

    def forward(self, feature, user_id, item_id):
        return self.fc(feature) + self.bias_users[user_id] + self.bias_items[item_id]
...
class NFM(nn.Module):
    def __init__(self, dim):
        ...
        self.fc = nn.Linear(dim, 1)
        # ------------------------------FM----------------------------------
        self.fm_V = nn.Parameter(torch.randn(dim, 16))
...
class FM(nn.Module):
    ...
    def init_weight(self):
        ...
        nn.init.uniform_(self.bias_users, a=0, b=0.1)
        nn.init.uniform_(self.bias_items, a=0, b=0.1)
    ...
    def forward(self, feature, uids, iids):
        ...
        return fm_out + self.bias_users[uids] + self.bias_items[iids]
